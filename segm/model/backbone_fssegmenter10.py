...
class Fssegmenter(nn.Module):
    ...
    def mask_feature(self, feature, support_mask):
        mask = F.interpolate(support_mask, feature.size()[2:], mode='bilinear',align_corners=True)
    ...
    def forward(self, query_img, support_imgs, support_masks):
        ...
        for i in range(self.shot):
            mask = support_masks[:, i, :, :].float().unsqueeze(1)
            ...
            supp_feat = self.down_feat(supp_feat)
            supp_feat_masked = self.mask_feature(supp_feat, mask)
            fore_feature = Weighted_GAP(supp_feat, mask)
            fore_feature_list.append(fore_feature)


        # B, S, C, h, w = support_imgs.shape
        query_feat = rearrange(query_feat, 'b c h w -> b (h w) c')
        supp_feat = rearrange(supp_feat, 'b c h w -> b (h w) c')
        supp_feat_masked = rearrange(supp_feat_masked, 'b c h w -> b (h w) c')
        fore_feature = rearrange(fore_feature, 'b c h w -> b (h w) c')

        similarity_q = self.get_similarity(query_feat,supp_feat_masked)
        similarity_s = self.get_similarity(supp_feat,supp_feat_masked)


        q_mask,masks_list_q = self.decoder(query_feat, fore_feature, (h, w),similarity_q)
        s_mask, _ = self.decoder(supp_feat, fore_feature, (h, w),similarity_s)
    ...
    def get_similarity(self,query_feat,supp_feat_masked):
        cosine_eps = 1e-7
        query_feat_norm = torch.norm(query_feat, 2, 2, True)
        supp_feat_masked_norm = torch.norm(supp_feat_masked, 2, 2, True)

        similarity = torch.bmm(query_feat, supp_feat_masked.transpose(1, 2)) / (
                torch.bmm(query_feat_norm, supp_feat_masked_norm.transpose(1, 2)) + cosine_eps)
        similarity = rearrange(similarity, 'b (hq wq) (hs ws) -> b 1 hq wq hs ws', hq=60, hs=60)
        similarity = self.conv_4d(similarity)
        similarity = rearrange(similarity, 'b 1 hq wq hs ws -> b (hq wq) hs ws')
        similarity = self.pool(similarity)
        similarity = rearrange(similarity, 'b hq_wq hs ws -> b hq_wq (hs ws)')

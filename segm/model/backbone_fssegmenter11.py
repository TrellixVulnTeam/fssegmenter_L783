...
class Fssegmenter(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(1, 1, 3, 1, 1),
                                   nn.Tanh(),
                                   nn.Conv2d(1, 1, 3, 1, 1),
                                   nn.Tanh(),
                                   )
        self.conv2 = nn.Sequential(nn.Conv2d(1, 1, 3, 1, 1),
                                   nn.Tanh(),
                                   nn.Conv2d(1, 1, 3, 1, 1),
                                   nn.Tanh(),
                                   )
        self.conv3 = nn.Sequential(nn.Conv2d(1, 1, 3, 1, 1),
                                   nn.Tanh(),
                                   nn.Conv2d(1, 1, 3, 1, 1),
                                   )
    ...
    def forward(self, query_img, support_imgs, support_masks):
        ...
        q_mask = self.upsample_mask(q_mask,h,w)
        s_mask = self.upsample_mask(s_mask,h,w)
        masks_list_q = [self.upsample_mask(masks,h,w) for masks in masks_list_q]

        return q_mask, s_mask, masks_list_q

    def upsample_mask(self,mask,h,w):
        mask = F.interpolate(mask, size=(mask.size(-1)*2,)*2, mode="bilinear",align_corners=True)
        mask = self.conv1(mask)
        mask = F.interpolate(mask, size=(mask.size(-1)*2,)*2, mode="bilinear",align_corners=True)
        mask = self.conv2(mask)
        mask = F.interpolate(mask, size=(h,w), mode="bilinear", align_corners=True)
        mask = self.conv3(mask)
        mask = torch.sigmoid(mask.squeeze(1))

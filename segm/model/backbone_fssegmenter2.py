...
class Fssegmenter(nn.Module):
    ...
    def forward(self, query_img, support_imgs, support_masks):
        ...
        for i in range(9,10):
            ...
            support_masked_feat = support_masked_feats[i]
    
            query_feat = rearrange(query_feat, 'b c h w -> b (h w) c')
            support_feat = rearrange(support_feat, 'b c h w -> b (h w) c')
            support_masked_feat = rearrange(support_masked_feat, 'b c h w -> b (h w) c')
    
            fore_feature = support_masked_feat.mean(1)
            q_mask, fore_feature_decoder = self.decoder(query_feat, fore_feature, (H, W))
            s_mask, _ = self.decoder(support_feat, fore_feature, (H, W))
        ...
        s_masks = torch.sigmoid(s_masks.squeeze(1))

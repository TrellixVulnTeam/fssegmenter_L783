...
class Fssegmenter(nn.Module):
    def __init__(
        ...
        self.res1 = nn.Sequential(
            nn.Conv2d(reduce_dim+self.decoder.n_layers-1, reduce_dim, kernel_size=1, padding=0, bias=False),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, query_img, support_imgs, support_masks):
        ...
        temp_query_feat = query_feat.clone()
        temp_supp_feat = query_feat.clone()
        ...
        s_mask, masks_list_s = self.decoder(supp_feat, fore_feature, (h, w))

        query_feat = torch.cat([temp_query_feat,q_mask]+masks_list_q, 1)
        query_feat = self.res1(query_feat)
        query_feat = self.res2(query_feat) + query_feat
        q_mask = self.cls(query_feat)

        supp_feat = torch.cat([temp_supp_feat, s_mask] + masks_list_s, 1)
        supp_feat = self.res1(supp_feat)
        supp_feat = self.res2(supp_feat) + supp_feat
        s_mask = self.cls(supp_feat)



        q_mask = F.interpolate(q_mask, size=(h, w), mode="bilinear")
        s_mask = F.interpolate(s_mask, size=(h, w), mode="bilinear")
        masks_list_q = [F.interpolate(masks, size=(h, w), mode="bilinear") for masks in masks_list_q]

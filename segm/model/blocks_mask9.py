...
class Attention(nn.Module):
    ...
    def forward(self, x, fore_feature,mask=None):
        ...
        x = (attn.transpose(2, 3) * v).transpose(1, 2).reshape(B, N, C)
        # x = self.proj(x)
        x = self.proj_drop(x)
...
class Block(nn.Module):
    def __init__(self, dim, heads, mlp_dim, dropout, drop_path):
        ...
        self.scale = dim ** -0.5
        # self.proj_patch = nn.Parameter(self.scale * torch.randn(dim, dim))
        # self.proj_classes = nn.Parameter(self.scale * torch.randn(dim, dim))
        self.mask_conv = nn.Sequential(nn.Conv2d(dim+256, int(dim/heads), 3, 1, 1),
    ...
    def forward(self, x, fore_feature,similarity, mask=None, return_attention=False):
        y, attn = self.attn(self.norm1(x), fore_feature,mask)
        if return_attention:
            return attn
        x = x + self.drop_path(y)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        masks = self.get_att_mask(x,fore_feature,similarity)
        return x,masks

...
from segm.engine import train_one_epoch \
    # , evaluate
from segm.val import evaluate
...
@click.option("--shot", default=1, type=int)
def main(
        log_dir,
        dataset,
        im_size,
    ...
    # val_kwargs["batch_size"] = dataset_kwargs["batch_size"]
    ...
    # checkpoint = torch.load(Path(r'seg_tiny_mask/checkpoint.pth'), map_location="cpu")
    # checkpoint_encoder = {k:v for k,v in checkpoint['model'].items() if 'encoder' in k}
    ...
    for epoch in range(start_epoch, num_epochs):
        # train for one epoch
        train_logger,train_q_miou,train_s_miou = train_one_epoch(
            model,
            train_loader,
            optimizer,
            lr_scheduler,
        ...
        if eval_epoch:
            q_miou,s_miou = evaluate(model, val_loader)
        ...
        if ptu.dist_rank == 0:
            ...
            train_stats['query_miou'] = train_q_miou.item()
            train_stats['support_miou'] = train_s_miou.item()

            val_stats = {}
            if eval_epoch:
                val_stats['query_miou'] = q_miou.item()
                val_stats['support_miou'] = s_miou.item()
                # val_stats = {
